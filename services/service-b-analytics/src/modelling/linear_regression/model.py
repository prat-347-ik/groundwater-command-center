...
class LinearRegressionModel(BaseGroundwaterModel):
    """
    Ordinary Least Squares (OLS) Linear Regression model implementation.
    Standard baseline model ported from the main branch.
    """
    model_type = "linear-regression"
    version = "1.0-ols"

    def __init__(self, region_id: str, features: Optional[List[str]] = None):
    ...
    def fit(self, df: pd.DataFrame, train_split_ratio: float = 0.80, min_history_days: int = 30, **kwargs) -> Dict[str, Any]:
        """
        Trains Linear Regression model using chronological time-based split.
        """
        ...
        if len(valid_indices) > 0:
            persistence_mae = float(mean_absolute_error(test_df.loc[valid_indices, self.target], persistence_pred.loc[valid_indices]))
        else:
            persistence_mae = mae
        ...
        logger.info(f"✅ Trained LinearRegression [{self.region_id}] | MAE: {mae:.4f} | RMSE: {rmse:.4f}")
    ...
    def predict_horizon(
        ...
        """
        Generates recursive multi-step forecasting for the region over horizon_days.
        """
        if not self.is_fitted:
            raise RuntimeError(f"LinearRegressionModel for region {self.region_id} is not fitted.")
        ...
        for i in range(1, horizon_days + 1):
            ...
            eff_rain = 0.0  # Conservative future rainfall assumption

            # Extraction schedule handling
    ...
    def predict_scenario(self, history_df: pd.DataFrame, rainfall_mod: float, extraction_mod: float) -> Dict[str, Any]:
        """
        Runs single-step sensitivity simulation using Linear Regression.
        """
        if not self.is_fitted:
            raise RuntimeError(f"LinearRegressionModel for region {self.region_id} is not fitted.")
    ...
    def get_feature_importance(self) -> List[Dict[str, Any]]:
        """
        Returns normalized coefficient magnitudes as feature importance ranking.
        """
        ...
        result = [
            {"feature": feat, "importance": float(round(norm_imp, 4))}
            for feat, norm_imp in zip(self.features, normalized)
        ]
    ...
    def save(self, filepath: str) -> None:
        ...
        logger.info(f"💾 Saved LinearRegressionModel artifact to {filepath}")
    ...
    def load(self, filepath: str) -> None:
        ...
        self.is_fitted = True
        logger.info(f"📂 Loaded LinearRegressionModel from {filepath}")
